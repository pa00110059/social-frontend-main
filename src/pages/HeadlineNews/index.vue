...
const getPageList = async () => {
    try {
        ...
        console.log("後端返回的完整 JSON：", result); // ✅ 確認 API 回傳的格式

        if (!result) {
            console.error("API 返回的 result 為 undefined！", result);
            return;
        }

        let pageInfo = result; // ✅ 直接使用 `result`，而不是 `result.data`
        console.log("解析後的 pageInfo：", pageInfo);

        if (!pageInfo.pageData) {
            console.error("pageData 為 undefined，請檢查 API 返回的數據！", pageInfo);
            return;
        }

        pageData.value = pageInfo.pageData || []; // ✅ 避免 `undefined`
        findNewsPageInfo.value.pageNum = pageInfo.pageNum || 1;
        findNewsPageInfo.value.pageSize = pageInfo.pageSize || 5;
        totalSize.value = +pageInfo.totalSize || 0;
    } catch (error) {
